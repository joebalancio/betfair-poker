...
public class Game {
    ...
    public void initGame() {
        ...
        System.out.println("dealerIndex "+dealerIndx);
        int nextTurnIdx = 0;
        ...
        System.out.println("smallBlindIdx "+smallBlindIdx);
        System.out.println("bigBlindIdx "+bigBlindIdx);
        // sub small & big blind
    ...
    public void playHand(int seatId, Action action, int currentBet) {
        ...
        if (currentSeat.isTurn()) 
        {
            ...
            System.out.println("Players to act "+playersToAct);
            if (null != currentSeat.getPlayer() && currentSeat.getPlayer().isAllIn()) 
            {
            ...
            if(playersToAct == 0)
            {
            	...
            	else if(GameStatus.FLOP.equals(status) || GameStatus.TURN.equals(status))
            	{
            		for(Seat seat: seats)
            		{
            			if(null != seat.getPlayer())
            ...
            {
            	int newSeat = seatId;
            	System.out.println("Seat id "+newSeat);
            	while(newSeat >= 0)
            	{
            		...
            		{
            			System.out.println("Next turn is for seatID "+ (newSeat + 1));
    ...
    public Set<Action> getAllowedActions(Player player) {
        ...
        System.out.println("Value of Min Bet "+bet);
        if (bet == 0) {
    ...
    private Map<HandValue, List<Seat>> getRankedPlayers()
    {
    	...
    	for (Seat seat : seats) {
    		...
    		if(null != seat.getPlayer())
    		{
        		...
        		System.out.println("Value for player "+seat.getPlayer().getName()+ " Is "+handValue.getValue() + " and type "+handValue.getType());
    ...
    public void payPots() 
    {
    	...
    	for (HandValue handValue : rankedPlayers.keySet()) {
    		...
    		System.out.println("winners are "+winners.size());
    		if (winners.size() == 1) {
    			//Single winner.
    			...
    			for (Seat seat : winners) {
    				//Determine the player's share of the pot.
    				System.out.println("Hole Cards "+seat.getPlayer().getHand().getHoleCards().toString());
    				System.out.println("Community Cards "+seat.getPlayer().getHand().getCommunityCards().toString());
...
*/
    ...
    public void dealCards() {
        // Burn the first card
        deck.dealCard();
        System.out.println("seat size "+getSeatSize());
        // Deal 2 cards to each player at the start
        for (Seat seat : seats) {
            Hand hand = new Hand();
            HoleCards holeCards = new HoleCards();
            System.out.println("how many cards dealt "+deck.dealCards(2).size());
    ...
    private void setPlayerTurn()
    {
    	...
    	else
    	{
    		...
    		System.out.println("Whose turn is next ?? "+nextTurnIdx);
    		seats.get(nextTurnIdx).setTurn(true);
    	}
    }
    ...
    public void turnOrRiver() {
        ...
        for (int i=0;i<seats.size();i++) {
        	...
        	{
        		//Hand hand = seat.getPlayer().getHand();
        		System.out.println("Com card size "+seat.getPlayer().getHand().getCommunityCards().size());	
