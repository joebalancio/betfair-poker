 ...
 * 
 * NOTE: This class is implemented with the focus on performance (instead of
 * clean design).
 * 
...
public class HandEvaluator {
    ...
    private void calculateDistributions() {
        for (Card card : cards) {
            rankDist[card.getRank().getRank()]++;
            suitDist[card.getSuit().getSuit()]++;
        }
    }
    ...
    private void findFlush() {
        for (int i = 0; i < Card.NO_OF_SUITS; i++) {
            if (suitDist[i] >= 5) {
                flushSuit = i;
                for (Card card : cards) {
                    if (card.getSuit().getSuit() == flushSuit) {
                        if (!wheelingAce || card.getRank() != Rank.ACE) {
                            flushRank = card.getRank().getRank();
                            break;
                        }
                    }
                }
    ...
    private void findStraight() {
        ...
        // 'wheeling Ace') .
        if ((count == 4) && (rank == Rank.FIVE.getRank())
                && (rankDist[Rank.ACE.getRank()] > 0)) {
            wheelingAce = true;
            straightRank = rank;
        }
    }
    ...
    private void calculateHighCard() {
        ...
        rankings[0] = type.getIntegerValue();
        // Get the five highest ranks.
        int index = 1;
        for (Card card : cards) {
            rankings[index++] = card.getRank().getRank();
            if (index > 5) {
                break;
            }
        }
    ...
    private boolean isStraightFlush() {
        if (straightRank != -1 && flushRank == straightRank) {
            ...
            if (inStraight >= 5 && inFlush >= 5) {
                if (straightRank == Rank.ACE.getRank()) {
                    // Royal Flush.
